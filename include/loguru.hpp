...
/*  Should be called from the main thread.
    ...
    That way you can set the default but have the user override it with the -v
   flag. Note that -v does not affect file logging (see loguru::add_file). You
   can you something other than the -v flag by setting the verbosity_flag
   option.
*/
LOGURU_EXPORT
...
/*  Will log to a file at the given path.
    ...
    To stop the file logging, just call loguru::remove_callback(path) with the
   same path.
*/
LOGURU_EXPORT
...
/*  Will be called right before abort().
    You can for instance use this to print custom error messages, or throw an
   exception.
...
/*  Add a string to be replaced with something else in the stack output.
   ...
   >&) You can clean it up with: auto verbose_type_name =
...
/* 	A stack trace gives you the names of the function at the point of a
   ...
   variables. Usage: void process_customers(const std::string& filename)
    {
...
/*
   ...
   thread. You can accomplish this thusly: void foo(const char* parameter)
    {
...
Here's how: some.hpp: namespace loguru { Text ec_to_text(MySmallType data) Text
ec_to_text(const MyBigType* data) } // namespace loguru some.cpp: namespace
loguru { Text ec_to_text(MySmallType small_value)
        {
            // Called only when needed, i.e. on a crash.
            std::string str = small_value.as_string(); // Format 'small_value'
here somehow. return Text{STRDUP(str.c_str())};
        }
        Text ec_to_text(const MyBigType* big_value)
        {
            // Called only when needed, i.e. on a crash.
            std::string str = big_value->as_string(); // Format 'big_value' here
somehow. return Text{STRDUP(str.c_str())};
        }
    } // namespace loguru
Any file that include some.hpp:
    void foo(MySmallType small, const MyBigType& big)
    {
        ...
        ERROR_CONTEXT("Big",   &big);  // `big` should not change during this
scope!
        ....
    }
*/
