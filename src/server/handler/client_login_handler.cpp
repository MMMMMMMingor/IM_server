...
void client_login_handler(Context &ctx, im_message::Message &in_message) {
    ...
    //获取内存中的信息
    user_msg *old_msg{nullptr};
    if (ctx.msg_map.find(pwd) == ctx.msg_map.end()) {
        ctx.msg_map[pwd] = new HistoryMessage();
    } else {
        // 判断之前的这个用户是否登录过， 如果有去获取历史信息
        if (ctx.users_info.count(username + pwd) != 0) {//判空
            old_msg = ctx.users_info[username + pwd];
            ctx.msg_map[pwd]->get_by_id(old_msg->m_msg_id);// 一定有消息的
        }
    }
