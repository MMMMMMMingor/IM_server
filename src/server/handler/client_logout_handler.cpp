...
void client_logout_handler(Context &ctx, im_message::Message &in_message) {
    ...
    std::string room_id =
            ctx.session_pool.find_session(session_id)->get_room_name();

    // 保存用户下线的信息, 目前只存消息的条数
    ctx.users_info[username + room_id] = new user_msg{ctx.session_pool.find_session(session_id)->get_msg_id()};
